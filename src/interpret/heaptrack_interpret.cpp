...
static AddressCache::SymbolCache cacheSymbols(Dwfl_Module* module, uint64_t elfStart, bool isArmArch)
{
    AddressCache::SymbolCache cache;
    ...
    for (int i = 0; i < numSymbols; ++i) {
        ...
        if (symbol) {
            ...
            cache.push_back({symAddr - elfStart, start, sym.st_size, symbol});
        }
    }
    return cache;
}
...
struct Module
{
    Module(string fileName, uintptr_t addressStart, Dwfl_Module* module, AddressCache* addressCache)
    ...
    AddressInformation resolveAddress(uintptr_t address) const
    {
        ...
        if (!addressCache->hasSymbolCache(fileName)) {
            ...
            addressCache->setSymbolCache(fileName, cacheSymbols(module, addressStart, isArmArch()));
        }

        auto cachedAddrInfo = addressCache->findSymbol(fileName, address - addressStart);
        if (!cachedAddrInfo.isValid()) {
            return info;
        }

        info.frame.function = cachedAddrInfo.symname;
...
private:
    Module* reportModule(const ModuleFragment& module)
    {
        ...
        ret = Module(module.fileName, module.addressStart, dwflModule, &m_addressCache);
        return &ret;
    }
